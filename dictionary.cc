...
void Dictionary::add_mis(const std::string& w) {
    int32_t h = find(w);
    mistokens_++;
    if (misword2int_[h] == -1) {
        entry e;
        ...
        e.count = 1;
        e.type = getType(w);
        mis_words_.push_back(e);
        misword2int_[h] = size_++;
    } else {
        mis_words_[misword2int_[h]].count++;
    }
}

void Dictionary::add_cor(const std::string& w) {
    int32_t h = find(w);
    cortokens_++;
    if (corword2int_[h] == -1) {
        entry e;
        ...
        e.count = 1;
        e.type = getType(w);
        cor_words_.push_back(e);
        corword2int_[h] = size_++;
    } else {
        cor_words_[corword2int_[h]].count++;
    }
}
...
const std::vector<std::string>& Dictionary::getSubstrings(int32_t i) const {
    assert(i >= 0);
    assert(i < nwords_);
    return words_[i].substrings;
}

const std::vector<int32_t>& Dictionary::getExcludedSubwords(int32_t i) const {
    assert(i >= 0);
    assert(i < nwords_);
    return words_[i].subwords;
}
...
const std::vector<std::string> Dictionary::getSubstrings(const std::string& word) const {
    int32_t i = getId(word);
    if (i >= 0) {
        return getSubstrings(i);
    }
    std::vector<int32_t> ngrams;
...
void Dictionary::initNgrams() {
  for (size_t i = 0; i < size_; i++) {
    ...
    if (words_[i].word != EOS) {
        computeSubwords(word, words_[i].subwords, words_[i].substrings);
        nngrams_ += words_[i].subwords.size();
    }
  }
}
...
bool Dictionary::readWord(std::istream& in, std::string& word) const {
  ...
  // trigger eofbit
  in.get();
  std::cerr << !word.empty() << std::endl;
  return !word.empty();
}


bool Dictionary::readP(std::istream& in, std::string& word) const {
    int c;
    std::streambuf& sb = *in.rdbuf();
    word.clear();
    while ((c = sb.sbumpc()) != EOF) {
...
int32_t Dictionary::readFromFile(std::istream& in) {
  std::string word;
  int64_t minThreshold = 1;
  int64_t wordCounts = 0;
...
int32_t Dictionary::readFromM(std::istream& in) {
    std::string wm;
    std::string we;
    int64_t minThreshold = 1;
    int64_t nPairs = 0;

    while (readWord(in, wm) && readWord(in, we)) {
        add(wm);
        //add_mis(wm);
        ...
        nPairs++;
        if (ntokens_ % 1000000 == 0 && args_->verbose > 1) {
            std::cerr << "\rRead " << ntokens_ / 1000000 << "M pairs" << std::flush;
        }
        //if (size_ > 0.75 * MAX_VOCAB_SIZE) {
        //    minThreshold++;
        //    threshold(minThreshold, minThreshold);
    ...
    if (args_->verbose > 0) {
        std::cerr << "\rRead " << ntokens_ / 1000000 << "M words" << std::endl;
        std::cerr << "Number of pairs:  " << nPairs << std::endl;
        std::cerr << "Number of labels: " << nlabels_ << std::endl;
    }
    if (size_ == 0) {
        throw std::invalid_argument(
...
int32_t Dictionary::getPairs(
        std::istream& in,
        std::vector<int32_t>& miswords,
    ...
    pairs.clear();
    miswords.clear();
    int32_t constraint = ceil(len / ratio);
    int32_t timer = 0;
    std::string wm;
    std::string we;


    //while ((timer < constraint) && (in >> wm >> we)) {
    while ((timer < constraint) && readP(in, wm) && readP(in, we)) {
        //std::cerr << wm << " : " << we << std::endl;
        int32_t hw = find(wm);
        int32_t he = find(we);
        int32_t widw = word2int_[hw];
        int32_t wide = word2int_[he];
        ...
        if ((getType(widw) == entry_type::word) && //&& !discard(widw, uniform(rng))) &&
            (getType(wide) == entry_type::word)) { //&& !discard(wide, uniform(rng)))) {

            pairs.push_back(std::make_pair(widw, wide));
            miswords.push_back(widw);
        }

        if (timer > MAX_LINE_SIZE || wm == EOS || we == EOS) {
        break;
        }
...
int32_t Dictionary::getAugmented(
        std::istream& in,
        std::istream& in_moe,
        std::vector<int32_t>& words,
        std::vector<std::pair<int32_t , int32_t >>& pairs,
    ...
    reset(in_moe);
    pairs.clear();
    int32_t constraint = ceil(double(ntokens) / double(ratio));
    int32_t timer = 0;
    std::string wm;
    std::string we;
    while ((timer < constraint) && (readWord(in_moe, wm)) && (readWord(in_moe, we))) {
        int32_t hw = find(wm);
        int32_t he = find(we);
        int32_t widw = word2int_[hw];
        int32_t wide = word2int_[he];
