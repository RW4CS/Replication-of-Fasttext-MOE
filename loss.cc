...
real BinaryLogisticLoss::binaryLogistic(
  ...
  if (labelIsPositive) {
      score = pow(score, ratio);
  } else {
      score = 1 - pow(1-score, ratio);
  }
  //std::cerr << "score value is : " << score << std::endl;
  if (backprop) {
    real alpha = lr * (real(labelIsPositive) - score);
    //std::cerr << "alpha value is : " << alpha << std::endl;
...
real OneVsAllLoss::forward(
  ...
  for (int32_t i = 0; i < osz; i++) {
    ...
    loss += binaryLogistic(i, state, isMatch, lr, backprop, backprop, ratio);
  }
...
real NegativeSamplingLoss::forward(
  ...
  //printf("original loss with true label and true backprop \n");
  real loss = binaryLogistic(target, state, true, lr, backprop, backprop, ratio);

  for (int32_t n = 0; n < neg_; n++) {
      //printf("original loss in negative sample with false label and true backprop \n");
    auto negativeTarget = getNegative(target, state.rng);
    loss += binaryLogistic(negativeTarget, state, false, lr, backprop, backprop, ratio);
  }
  return loss;
}


real NegativeSamplingLoss::forward_moe(
        const std::vector<int32_t>& miswords,
        int32_t index,
        int32_t me,
        Model::State& state,
    ...
    //printf("MOE loss with true label and true backprop \n");
    real loss = binaryLogistic(me, state, true, lr, false, backprop, ratio);

    for (int32_t n = 0; n < neg_; n++) {
        auto negativeTarget = getNegativeMOE(miswords[index], me, state.rng);
        //std::cerr << "Reach inner " << n <<"sample\n";
        //printf("MOE loss in NE samples with false label and true backprop \n");
        auto ng = dict->getSubwords(negativeTarget).front();
        loss += binaryLogistic(ng, state, false, lr, backprop, backprop, ratio);
    }
    return loss;
}
...
int32_t NegativeSamplingLoss::getNegativeMOE(
        int32_t target,
        int32_t me,
        std::minstd_rand& rng) {
    int32_t negative;
    ...
    do {
        negative = negatives_[uniform_(rng)];
        times++;
    } while ((target == negative || target == me) && times <= 20);
    return negative;
}
...
real HierarchicalSoftmaxLoss::forward(
  ...
  for (int32_t i = 0; i < pathToRoot.size(); i++) {
    loss += binaryLogistic(pathToRoot[i], state, binaryCode[i], lr, backprop, backprop, ratio);
  }
  return loss;
}

real HierarchicalSoftmaxLoss::forward_moe(
        const std::vector<int32_t>& miswords,
        int32_t index,
        int32_t me,
        Model::State& state,
