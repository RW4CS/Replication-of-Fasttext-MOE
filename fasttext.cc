...
void FastText::getWordVector(Vector& vec, const std::string& word) const {
  ...
  if (ngrams.size() > 0) {
    vec.mul(1.0 / (ngrams.size()-1));
  }
  //std::cerr << "word is : ---  " << word << "  ---" << " and ngram has " << ngrams.size() << std::endl;
}
...
void FastText::saveVectors(const std::string& filename) {
  ...
  ofs << dict_->nwords()+dict_->nngrams() << " " << args_->dim << std::endl;
  Vector vec(args_->dim);
  std::map<std::string, int32_t> viewed;
  for (int32_t i = 0; i < dict_->nwords(); i++) {
      std::string word = dict_->getWord(i);
      getWordVector(vec, word);
      ofs << word << " " << vec << std::endl;
      const std::vector<int32_t>& ngrams = dict_->getSubwords(word);
      for (int32_t j = 0; j < ngrams.size()-1; j++) {
          vec.zero();
          std::map<std::string, int32_t>::iterator it;
          it = viewed.find(dict_->getSubstrings(word)[j]);
          if (it != viewed.end()) {
              continue;
          } else {
              addInputVector(vec, ngrams[j+1]);
              ofs << dict_->getSubstrings(word)[j] << " " << vec << std::endl;
              viewed.emplace(dict_->getSubstrings(word)[j], 1);
          }
      }
  }
  ofs.close();
...
void FastText::save2Vectors(const std::string& filename1, const std::string& filename2) {
    if (!input_ || !output_) {
        throw std::runtime_error("Model never trained");
    }
    ...
    if (!ofs.is_open()) {
        throw std::invalid_argument(
                filename1 + " cannot be opened for saving vectors!");
    }
    if (!ofs2.is_open()) {
        throw std::invalid_argument(
                filename2 + " cannot be opened for saving vectors!");
    }
    ofs2 << dict_->nwords() << " " << args_->dim << std::endl;
    ofs << dict_->nwords()+dict_->nngrams() << " " << args_->dim << std::endl;
    Vector vec(args_->dim);
    std::map<std::string, int32_t> viewed;
    for (int32_t i = 0; i < dict_->nwords(); i++) {
...
void FastText::skipgram(
  ...
  for (int32_t w = 0; w < line.size(); w++) {
    ...
    for (int32_t c = -boundary; c <= boundary; c++) {
      if (c != 0 && w + c >= 0 && w + c < line.size()) {
        model_->update_ori(ngrams, line, w + c, lr, state, 1-alpha, ratio, dict_);
      }
    }
  }
  //std::cerr << miswords.size() << std::endl;
  for (int32_t m = 0; m < miswords.size(); m++) {
      const std::vector<int32_t>& ngrams_temp = dict_moe->getSubwords(miswords[m]); //getExcludedSubwords(miswords[m]);
      //for (int32_t i = 0; i < ngrams_temp.size(); i++) {
      //    if (std::isnan(ngrams_temp[i])) {
      //        printf("problems \n");
      //    }
      //}

      std::vector<int32_t> ng;
      for (int32_t i = 1; i < ngrams_temp.size(); i++) {
          ng.push_back(ngrams_temp[i]);
      }
      const std::vector<int32_t>& ngrams = ng;

      int32_t me = dict_moe->getSubwords(pairs[m].second).front();
      //printf("Reach here 1\n");
      //std::cerr << m << std::endl;
      model_->update_moe(ngrams, miswords, me, m, lr, state, alpha, ratio, dict_moe);
  }
}
...
void FastText::trainThread(int32_t threadId, const TrainCallback& callback) {
  ...
  std::ifstream ifs_moe(args_->moe);
  utils::seek(ifs, threadId * utils::size(ifs) / args_->thread);

  Model::State state(args_->dim, output_->size(0), threadId + args_->seed);

  const int64_t ntokens = dict_->ntokens();
  const int64_t moetokens = 0;//dict_moe->ntokens();
  int64_t localTokenCount = 0;
  int64_t moeTokenCount = 0;
  std::vector<int32_t> line, labels;
  std::vector<int32_t> miswords, moe_labels;
  std::vector<std::pair<int32_t , int32_t >> pairs;
  uint64_t callbackCounter = 0;

  real alpha = 0.05;
  real VM_ratio = (real)Vocab_V / Vocab_M;
    ...
    while (keepTraining(ntokens)) {
      ...
      if (args_->model == model_name::sup) {
        ...
        cbow(state, lr, line);
          ...
          int64_t temp_num = 0;
        temp_num = dict_->getLine(ifs, line, state.rng);
        localTokenCount += temp_num;
        moeTokenCount += dict_moe->getPairs(ifs_moe, miswords, pairs, temp_num, VM_ratio, state.rng);

        skipgram(state, lr, line, alpha, VM_ratio, miswords, pairs);
      }
...
std::shared_ptr<Matrix> FastText::getInputMatrixFromFile(
  ...
  std::shared_ptr<DenseMatrix> input = std::make_shared<DenseMatrix>(
      dict_->nwords() + dict_->nngrams(), args_->dim);
...
std::shared_ptr<Matrix> FastText::createRandomWordMatrix() const {
    std::shared_ptr<DenseMatrix> input = std::make_shared<DenseMatrix>(
...
void FastText::train(const Args& args, const TrainCallback& callback) {
  ...
  dict_moe = std::make_shared<Dictionary>(args_);

  if (args_->input == "-") {
    // manage expectations
    throw std::invalid_argument("Cannot use stdin for training!");
  }
  std::cerr << args_->input << std::endl;
  std::cerr << args_->moe << std::endl;
  ...
  if (!ifs.is_open()) {
    ...
    std::cerr << "Read from text file \n" << std::endl;

  Vocab_V = dict_->readFromFile(ifs);
  ifs.close();

  std::ifstream ifs_moe(args_->moe);
  if (!ifs_moe.is_open()) {
      throw std::invalid_argument(
              args_->moe + " cannot be opened for training!");
  }
  std::cerr << "Read from MOE file 2 \n" << std::endl;

  Vocab_M = dict_moe->readFromM(ifs_moe);
  ifs_moe.close();

  printf("=====================================\n");
  printf("\n");
  std::cerr << "number of words read in corpus:" << Vocab_V << std::endl;
  std::cerr << "number of pairs read in M:" << Vocab_M << std::endl;
  printf("\n");
  printf("=====================================\n");
  if (!args_->pretrainedVectors.empty()) {
    ...
    input_word_ = createRandomWordMatrix();
  }
