...
real DenseMatrix::dotRow(const Vector& vec, int64_t i) const {
  ...
  if (i >= m_) {
      ...
      for (int64_t j = 0; j < n_; j++) {
          if (std::isnan(at(i, j))) {
              std::cerr << "at(i, j) is NaN with i = " << i << " and j is " << j << std::endl;
          }
          if (std::isnan(vec[j])) {
              std::cerr << "vec[j] is NaN with j = " << j << std::endl;
          }
          d += at(i, j) * vec[j];
      }
  }
...
void DenseMatrix::addRowToVector2(Vector& x, int32_t i) const {
    assert(i >= 0);
    assert(i < this->size(0));
    //std::cerr << this->size(0) << std::endl;
    assert(x.size() == this->size(1));
    for (int64_t j = 0; j < n_; j++) {
        //std::cerr << i << std::endl;
        x[j] += at(i, j);
